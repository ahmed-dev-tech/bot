...
setPair=input("Enter pair e.g. XBTUSD: ")
kraken_trade_struct = ['price', 'volume', 'date', 'type', 'market_limit', 'misc']
#candle_struct = ['open', 'close', 'high', 'low', 'date']
trades = {}

last = input("Enter Timestamp e.g 0 , 1616663618 or go to google and get date converted from timestamp: ")
try:
	while True:
		print('https://api.kraken.com/0/public/Trades?pair={setPair}&since={last}'.format(setPair=setPair,last=last))
		with urllib.request.urlopen('https://api.kraken.com/0/public/Trades?pair={setPair}&since={last}'.format(setPair=setPair,last=last)) as url:
		#with urllib.request.urlopen('https://poloniex.com/public?command=returnChartData&currencyPair=BTC_ETH&start='+str(start_epoch)+'&end='+str(end_epoch)+'&period='+str(low_interval)) as url:
			...
			if(len(data['error']) != 0):
				...
				trade_df.to_pickle('kraken_{setPair}.pickle'.format(setPair=setPair))
				print('pickle saved!')
				exit()
